...
<template>
  <form @submit.prevent="signUp">
    <div class="w-[328px] md:w-[400px] font-sans border-0 text-lg">
      ...
      <BaseInput v-model="newUser.firstName" text="First name" :error="error.firstName || ''"/>
      <BaseInput v-model="newUser.lastName" text="Last name" :error="error.lastName || ''"/>
      <BaseInput v-model="newUser.email" text="Email address" :error="error.email || ''"/>
      <BaseInput v-model="newUser.password" type="password" :error="error.password || ''" text="Password" />
